...
public class GenericElements extends ObjectRepo {
	...
	public static WebElement getTestOrProdLocator(WebElement testElement,WebElement prodElement){
	  ...
	  public static void SelectFilterCheckBox(String checkboxName) throws Exception {
			HubbellHomePage hp = new HubbellHomePage(driver);
			WebElement element =driver.findElement(By.xpath("//*[text()='"+checkboxName+"']//preceding-sibling::*"));
			ButtonHelper.click(element, checkboxName);
	  }
	  
	  public static void selectByGroup(String groupByText) throws Exception {
		  HubbellHomePage hp = new HubbellHomePage(driver);
			 for(WebElement el : hp.groupBy) {
				String text =  el.getText();
				if(text.equals(groupByText)) {
					ButtonHelper.click(el, groupByText);
					break;
				}
			 }
	  }
